...
// htmlTag 去掉 HTML 标签，与 Python 的 re.sub(r'<[^>]+>', ' ', text) 同规则。
...
//
// 字符集或传输编码不认识时 message.Read 仍然给得出可读的 Entity，这里照 Python
// errors='ignore' 的路子接着用：认不出的字节变成替换符，总好过整封邮件被丢掉。
func parseMessage(raw []byte) (parsedMessage, error) {
	ent, err := message.Read(bytes.NewReader(raw))
	if ent == nil {
		// 邮件头坏到解析不出来时，把整封原文当正文接着扫——关键词照样能命中，
		// 与 Python 版 message_from_bytes"带缺陷也给个对象"的结果一致。
...
// 与 Python 版 decode_header 失败时返回原串一致。
